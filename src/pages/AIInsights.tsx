...
export default function AIInsights() {
  ...
  const filteredByFilters = papers.filter((p: Paper) => {
  ...
  const manuallySelected = papers.filter((p) => manuallySelectedIds.includes(p.id));
  const selectedPapers = [
    ...filteredByFilters,
    ...manuallySelected.filter((p) => !filteredByFilters.some((fp) => fp.id === p.id)),
  ];
          ...
          <div className="space-y-4">
            ...
            <div>
              <button
                ...
                className="flex items-center gap-1.5 text-xs font-medium text-gray-600 hover:text-cyan-600"
              >
                <ChevronRight className={cn('w-3.5 h-3.5 transition-transform', showManualSelect && 'rotate-90')} />
                Also include specific papers manually
                {manuallySelectedIds.length > 0 && (
                  <span className="ml-1 px-1.5 py-0.5 rounded-full bg-cyan-100 text-cyan-700 text-[10px] font-semibold">
                    {manuallySelectedIds.length}
                  </span>
                )}
              </button>
              {showManualSelect && (
                <div className="mt-2 border border-gray-100 rounded-xl max-h-48 overflow-auto">
                  {papers.map((p) => (
                    <label key={p.id} className="flex items-center gap-2 px-3 py-2 hover:bg-gray-50 cursor-pointer">
                      <input
                        type="checkbox"
                        checked={manuallySelectedIds.includes(p.id)}
                        onChange={(e) =>
                      ...
                      {p.year && <span className="text-xs text-gray-400 flex-shrink-0">{p.year}</span>}
                    </label>
                  ))}
                </div>
              )}
